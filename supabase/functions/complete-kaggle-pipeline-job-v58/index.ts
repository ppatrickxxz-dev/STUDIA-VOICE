...
async function kickNext(supabaseUrl:string,adminKey:string){
  try{
    const response=await fetch(`${supabaseUrl}/functions/v1/compute-kaggle-v58`,{
      method:'POST',
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(job.job_type!=='music_generation'||!ACTIVE_MUSIC_STATUSES.includes(String(job.status)))return json({ok:false,error:'job_not_active_music',status:job.status},409)
    ...
    const metadata={engine:'ACE-Step 1.5',model:ACE_MODEL,model_revision:ACE_REVISION,worker:'kaggle_ticketed',provider:'kaggle',purpose:'generated_reference_mix',generation_seed:Number.isFinite(seed)?seed:null,generation_shift:executedShift,generation_dtype:generationDtype,dtype_patch_sha256:dtypePatchSha,bpm:Number(p.bpm)||null,keyscale:String(p.keyscale||''),instrumental:Boolean(p.instrumental),proof_version:'pablovoice_native_music_v2_4_queue'}
