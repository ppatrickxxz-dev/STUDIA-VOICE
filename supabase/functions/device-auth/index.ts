...
function cors(origin=''){
 const allowed=origin===PROJECT||CLOUDFLARE_ORIGIN.test(origin)
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(req.method==='GET'){
   ...
   if(u.searchParams.get('health')==='1')return json(origin,{ok:true,service:'device-auth',version:'8.0-cloudflare-direct',candidate:STUDIO,ui_transport:'cloudflare-workers-static-assets',edge_html:false,vercel_dependency:false,preview_slot_dependency:false,bootstrap_pairing:true,oidc_signing:true,oidc_b09:true,origins:[STUDIO,PROJECT]})
