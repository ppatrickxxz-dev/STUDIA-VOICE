...
async function automaticDevice(admin:any,pubClient:any,origin:string,label:string){
 ...
 const {data:created,error:createError}=await admin.auth.admin.createUser({email,password,email_confirm:true,app_metadata:{pablovoice_app_device:true,access_profile:'unified_online'}})
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(req.method==='GET'){
   ...
   if(u.searchParams.get('health')==='1')return json(origin,{ok:true,service:'device-auth',version:'9.0-unified-online',candidate:STUDIO,ui_transport:'cloudflare-workers-static-assets',edge_html:false,vercel_dependency:false,preview_slot_dependency:false,bootstrap_pairing:true,transparent_device_access:true,user_login_ui:false,password_prompt:false,offline_mode:false,oidc_signing:true,oidc_b09:true,origins:[STUDIO,ANDROID_APP_ORIGIN,PROJECT]})
  ...
  if(action==='auto'){const out=await automaticDevice(admin,pubClient,origin,String(body?.label||'PabloVoice'));return json(origin,out.body,out.status)}
