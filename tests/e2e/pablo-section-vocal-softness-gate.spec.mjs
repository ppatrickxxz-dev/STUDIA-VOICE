...
async function seedProject(page) {
  await page.evaluate(async () => {
    ...
    vocal.regionAutomation.push(
      { id: `manual_high:${project.arrangementMap.sections[0].id}`, kind: 'high_shelf', startSeconds: 0.3, endSeconds: 0.95, gainDb: 1, frequencyHz: 8000, q: 1, confidence: 1, source: 'user_manual', enabled: true },
      { id: `manual_peak:${project.arrangementMap.sections[0].id}`, kind: 'peaking_eq', startSeconds: 0.3, endSeconds: 0.95, gainDb: 1, frequencyHz: 900, q: 1.1, confidence: 1, source: 'user_manual', enabled: true },
    );
  ...
  await page.evaluate(() => {
    ...
    if (!Context.prototype.__pvOriginalCreateBiquadFilter) {
      ...
      Context.prototype.createBiquadFilter = function patchedCreateBiquadFilter(...args) {
        ...
        globalThis.__pvSoftnessBiquads = globalThis.__pvSoftnessBiquads || [];
        globalThis.__pvSoftnessBiquads.push(node);
        return node;
      };
    }
    globalThis.__pvSoftnessBiquads = [];
  });
}

async function resetFilterEvidence(page) {
  await page.evaluate(() => { globalThis.__pvSoftnessBiquads = []; });
}

async function filterEvidence(page) {
  return page.evaluate(() => (globalThis.__pvSoftnessBiquads || []).map((node) => ({
    type: node.type,
...
async function persistedSoftness(page) {
  return page.evaluate(async () => {
    const storage = await import('./storage.mjs');
    const project = await storage.getProject(storage.activeProjectSessionId());
    const vocal = project.tracks.find((track) => track.kind === 'recording');
    const event = vocal.regionAutomation.find((item) => item.source === 'pablo_section_vocal_softness');
    return {
      event,
      sources: vocal.regionAutomation.map((item) => item.source).sort(),
    };
  });
}

test('WEB VOCAL SOFTNESS GATE: darken and deharsh are real subtractive EQ, A/B safe and selectively undoable', async ({ page }) => {
  const errors = [];
  ...
  await page.locator('[data-form="new-project"] input[name="name"]').fill('Gate suavização vocal regional');
  await page.locator('[data-form="new-project"]').getByRole('button', { name: 'Criar' }).click();
  await page.locator('#audio-picker').setInputFiles({ name: 'softness.wav', mimeType: 'audio/wav', buffer: wavFixture() });
  await expect(page.getByText('softness.wav').first()).toBeVisible();
  await seedProject(page);
  await page.locator('[data-route="pablo"]').first().click();

  await sendPablo(page, 'deixa minha voz com menos brilho 6 dB no refrão');
  await expect(page.getByText(/só aplico até 4 dB automaticamente/i).last()).toBeVisible();

  await sendPablo(page, 'deixa minha voz com menos brilho no refrão');
  await expect(page.getByText(/high-shelf de -2 dB a partir de 6\.5 kHz/i).last()).toBeVisible({ timeout: 10_000 });
  await expect(page.getByText(/Studio · suavização regional salva/i).last()).toBeVisible();
  let saved = await persistedSoftness(page);
  expect(saved.event.kind).toBe('high_shelf');
  expect(saved.event.frequencyHz).toBe(6500);
  expect(saved.event.gainDb).toBe(-2);
  expect(saved.sources).toEqual(['pablo_section_vocal_softness', 'user_manual', 'user_manual']);

  await installFilterEvidence(page);
  await sendPablo(page, 'compara o refrão');
  let panels = page.locator('[data-section-mix-ab]');
  await expect(panels).toHaveCount(1);
  let panel = panels.last();
  await panel.getByRole('button', { name: 'Ouvir A' }).click();
  await page.waitForTimeout(120);
  let a = await filterEvidence(page);
  expect(a.some((item) => item.type === 'highshelf' && item.frequencyHz === 8000)).toBe(true);
  expect(a.some((item) => item.type === 'highshelf' && item.frequencyHz === 6500 && item.gainDb < -1.5)).toBe(false);

  await resetFilterEvidence(page);
  await panel.getByRole('button', { name: 'Ouvir B' }).click();
  await page.waitForTimeout(120);
  let b = await filterEvidence(page);
  expect(b.some((item) => item.type === 'highshelf' && item.frequencyHz === 8000)).toBe(true);
  expect(b.some((item) => item.type === 'highshelf' && item.frequencyHz === 6500 && item.gainDb < -1.5)).toBe(true);
  ...
  await sendPablo(page, 'deixa minha voz menos estridente no refrão');
  await expect(page.getByText(/EQ de -1\.5 dB em 3\.8 kHz/i).last()).toBeVisible({ timeout: 10_000 });
  saved = await persistedSoftness(page);
  expect(saved.event.kind).toBe('peaking_eq');
  expect(saved.event.frequencyHz).toBe(3800);
  expect(saved.event.q).toBe(1.15);
  expect(saved.event.gainDb).toBe(-1.5);
  expect(saved.sources).toEqual(['pablo_section_vocal_softness', 'user_manual', 'user_manual']);

  await resetFilterEvidence(page);
  await sendPablo(page, 'compara o refrão');
  panels = page.locator('[data-section-mix-ab]');
  await expect(panels).toHaveCount(2);
  panel = panels.last();
  await panel.getByRole('button', { name: 'Ouvir A' }).click();
  await page.waitForTimeout(120);
  a = await filterEvidence(page);
  expect(a.some((item) => item.type === 'peaking' && item.frequencyHz === 900)).toBe(true);
  expect(a.some((item) => item.type === 'peaking' && item.frequencyHz === 3800 && item.gainDb < -1)).toBe(false);

  await resetFilterEvidence(page);
  await panel.getByRole('button', { name: 'Ouvir B' }).click();
  await page.waitForTimeout(120);
  b = await filterEvidence(page);
  expect(b.some((item) => item.type === 'peaking' && item.frequencyHz === 900)).toBe(true);
  expect(b.some((item) => item.type === 'peaking' && item.frequencyHz === 3800 && item.gainDb < -1)).toBe(true);

  await sendPablo(page, 'desfaz a suavização da minha voz no refrão');
  await expect(page.getByText(/Desfiz a suavização vocal que eu tinha criado no Refrão/i).last()).toBeVisible({ timeout: 10_000 });
  ...
  expect(afterUndo).toEqual([
    { source: 'user_manual', kind: 'high_shelf', frequencyHz: 8000 },
    { source: 'user_manual', kind: 'peaking_eq', frequencyHz: 900 },
  ]);
