...
test('WEB VOCAL PRESENCE GATE: peaking EQ is regional, A/B isolated and selectively undoable', async ({ page }) => {
  const errors = [];
  ...
  await page.locator('[data-form="new-project"] input[name="name"]').fill('Gate presença vocal regional');
  await page.locator('[data-form="new-project"]').getByRole('button', { name: 'Criar' }).click();
  await page.locator('#audio-picker').setInputFiles({ name: 'presence.wav', mimeType: 'audio/wav', buffer: wavFixture() });
  await expect(page.getByText('presence.wav').first()).toBeVisible();
  await seedProject(page);

  await page.locator('[data-route="pablo"]').first().click();
  await sendPablo(page, 'traz minha voz mais pra frente 6 dB no refrão');
  await expect(page.getByText(/só aplico até \+3,5 dB de EQ regional/i).last()).toBeVisible();

  await sendPablo(page, 'deixa minha voz mais presente só no refrão');
  await expect(page.getByText(/presença de \+1\.8 dB em 3\.2 kHz/i).last()).toBeVisible({ timeout: 10_000 });
  await expect(page.getByText(/Studio · presença regional salva/i).last()).toBeVisible();
  ...
  const saved = await page.evaluate(async () => {
    ...
    const event = vocal.regionAutomation.find((item) => item.source === 'pablo_section_vocal_presence');
    vocal.regionAutomation.push({
      id: `manual_presence_peaking:${project.arrangementMap.sections[0].id}`,
      kind: 'peaking_eq', startSeconds: 0.3, endSeconds: 0.95,
      gainDb: 0.8, frequencyHz: 900, q: 1.05, confidence: 1, source: 'user_manual', enabled: true,
    });
    await storage.saveProject(project);
    return { event, supportCount: support.regionAutomation.length };
  });
  expect(saved.event.kind).toBe('peaking_eq');
  expect(saved.event.frequencyHz).toBe(3200);
  expect(saved.event.q).toBe(0.9);
  expect(saved.event.gainDb).toBe(1.8);
  expect(saved.event.startSeconds).toBe(0.3);
  expect(saved.event.endSeconds).toBe(0.95);
  expect(saved.supportCount).toBe(0);

  await sendPablo(page, 'mais clareza na minha voz no refrão');
  const repeatedCount = await page.evaluate(async () => {
    const storage = await import('./storage.mjs');
    const project = await storage.getProject(storage.activeProjectSessionId());
    return project.tracks.find((track) => track.kind === 'recording').regionAutomation.filter((event) => event.source === 'pablo_section_vocal_presence').length;
  });
  ...
  expect(a.some((item) => item.type === 'peaking' && item.frequencyHz === 3200)).toBe(false);
  ...
  expect(persistedAfterA).toEqual(['pablo_section_vocal_presence', 'user_manual']);
  ...
  expect(b.some((item) => item.type === 'peaking' && item.frequencyHz === 3200 && item.gainDb > 1.7)).toBe(true);

  await sendPablo(page, 'desfaz a presença da minha voz no refrão');
  await expect(page.getByText(/Desfiz a presença vocal que eu tinha criado no Refrão/i).last()).toBeVisible({ timeout: 10_000 });
  ...
  expect(afterUndo).toEqual([{ source: 'user_manual', kind: 'peaking_eq', frequencyHz: 900, q: 1.05 }]);
