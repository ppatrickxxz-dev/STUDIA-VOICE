...
test('WEB SECTION VOCAL GAIN GATE: Pablo boosts only the identified vocal inside a complete confirmed chorus', async ({ page }) => {
  const errors = [];
  ...
  await page.locator('[data-form="new-project"] input[name="name"]').fill('Gate ganho vocal por seção');
  ...
  const afterUnsafe = await page.evaluate(async () => {
    const storage = await import('./storage.mjs');
    const project = await storage.getProject(storage.activeProjectSessionId());
    return project.tracks.map((track) => track.regionAutomation.length);
  });
  expect(afterUnsafe).toEqual([0, 0]);
  ...
  expect(saved.activeTrackId).not.toBe(saved.vocalId);
  expect(saved.vocalEvents).toHaveLength(1);
  expect(saved.instrumentalEvents).toHaveLength(0);
  expect(saved.event.kind).toBe('gain');
  expect(saved.event.startSeconds).toBe(0.3);
  expect(saved.event.endSeconds).toBe(0.95);
  expect(saved.event.gainDb).toBe(2);
  expect(saved.event.confidence).toBe(1);
  expect(saved.event.enabled).toBe(true);
  expect(saved.labels.some((label) => /Voz \+2 dB no Refrão/.test(label))).toBe(true);
