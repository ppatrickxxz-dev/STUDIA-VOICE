...
async function seedHqChorus(page) {
  await page.evaluate(async () => {
    const storage = await import('./storage.mjs');
    ...
    const referenceTrack = core.createTrack({
      name: 'Demo IA HQ · Take 1',
      assetId: 'asset_hq_gate_1',
      type: 'audio/mpeg',
      duration: 60,
      sampleRate: 48000,
      channels: 2,
    ...
    project.songCreation = {
      latestTakeId: 'songtake_hq_gate_1',
      takes: [{
        id: 'songtake_hq_gate_1',
        providerSongId: 'song_provider_gate_1',
...
test('MUSICAL PLAN REVIEW GATE: section regeneration is reviewable in chat and does not call provider before click', async ({ page }) => {
  ...
  await page.goto('/', { waitUntil: 'networkidle' });
  await expect(page.locator('.pv-nav')).toBeVisible({ timeout: 10_000 });
  await createProject(page, 'Gate Regeneração Conversacional');
  await seedHqChorus(page);
  ...
  await expect(generationMessage.locator('[data-musical-plan-detail]')).toContainText(/Regeneração por seção preparada/i);
  await expect(generationMessage.locator('[data-musical-generation-target]')).toContainText(/Refrão · 0:10 · origem: seção confirmada/i);
  await expect(generationMessage.getByRole('button', { name: 'Gerar nova versão' })).toBeVisible();
  await expect(page.getByText(/Não consegui usar essa análise: dados insuficientes/i)).toHaveCount(0);
