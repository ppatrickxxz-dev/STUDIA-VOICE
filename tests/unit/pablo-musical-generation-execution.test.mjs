...
function projectWithTake({ twoChoruses = false } = {}) {
  ...
  project.songCreation = {
    latestTakeId: 'songtake_hq_1',
    takes: [{
      id: 'songtake_hq_1',
      providerSongId: 'song_provider_abc',
      referenceTrackId: 'track_hq_1',
      durationSeconds: 60,
      bpm: 105,
      key: 'A',
      mode: 'minor',
  ...
  project.activeTrackId = 'track_hq_1';
  return project;
}

test('reviewed generation resolves a unique confirmed chorus and preserves unselected ranges', async () => {
...
test('multiple choruses fail closed without context and resolve from a recent playhead when unambiguous', async () => {
...
test('review binds generation to the source take identity and rejects stale song-id state', async () => {
  const project = projectWithTake();
  const review = await buildMusicalPlanReview('abre mais o refrão sem mexer no resto', project);
...
test('reviewed section regeneration executes provider only after review and persists a new take result', async () => {
  const project = projectWithTake();
  const review = await buildMusicalPlanReview('abre mais o refrão sem mexer no resto', project);
  let providerCalls = 0;
  let persistCalls = 0;
  ...
  const persist = async (sourceProject, plan, result) => {
    ...
    return {
      ...
      track: { id: 'track_hq_2' },
      take: { id: 'songtake_hq_2', providerSongId: result.songId },
      takeNumber: 2,
      assetId: 'asset_hq_2',
    };
  };
