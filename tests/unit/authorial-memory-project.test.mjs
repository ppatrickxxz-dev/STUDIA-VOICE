...
test('authorial feedback extends existing project memory as evidence', () => {
  const result = respondToAuthorialFeedback('Evite a palavra futuro', {
    authorialMemory: {
      ...
      vocabulary: ['malícia'],
      avoid: ['promessa'],
      acceptedPatterns: ['term:malícia'],
      rejectedPatterns: ['term:promessa'],
      evidenceCount: 2,
    },
  });
  assert.equal(result.supported, true);
  assert.deepEqual(result.authorialMemory.avoid.sort(), ['futuro', 'promessa']);
  assert.equal(result.authorialMemory.evidenceCount, 3);
});

test('conversation boundary persists feedback but never rewrites the lyric itself', async () => {
  let persisted = null;
  let audioCalls = 0;
  const result = await executePabloAudioMessage('Não use a palavra promessa', {
    projectId: 'p1',
    lyrics: 'Sem promessa pro futuro',
    authorialMemory: null,
  }, {
    audioToolRuntime: async () => { audioCalls += 1; return { ok: true }; },
    persistAuthorialMemory: async (memory, feedback) => {
      persisted = { memory, feedback };
      return { ok: true, projectId: 'p1', evidenceCount: memory.evidenceCount };
    },
  });
  assert.equal(result.kind, 'pmi_authorial_feedback');
  assert.equal(result.canApply, true);
  assert.equal(result.execution, 'allowed');
  assert.equal(audioCalls, 0);
  assert.deepEqual(persisted.memory.avoid, ['promessa']);
...
test('project schema v5 bounds and snapshots authorial memory', () => {
  const migrated = migrateProject({
    ...
    authorialMemory: {
      schema: 'anything',
      vocabulary: Array.from({ length: 100 }, (_, index) => `termo-${index}`),
      avoid: ['promessa'],
      acceptedPatterns: ['term:malícia'],
      rejectedPatterns: ['term:promessa'],
      evidenceCount: 3,
