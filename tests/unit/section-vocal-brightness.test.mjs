...
} from '../../packages/core/src/section-vocal-brightness.mjs';
...
test('parses a simple brightness request and blocks unsafe shelf gain', () => {
  assert.deepEqual(parseSectionVocalBrightnessCommand('dá mais brilho à minha voz só no refrão'), {
    section: 'chorus', label: 'Refrão', occurrence: null,
    gainDb: 2.5, frequencyHz: 6500, blocked: false,
  });
  const unsafe = parseSectionVocalBrightnessCommand('coloca brilho na minha voz 8 dB no refrão');
  assert.equal(unsafe.blocked, true);
  assert.equal(unsafe.reason, 'brightness_out_of_safe_range');
  assert.equal(unsafe.requestedGainDb, 8);
});

test('applies one idempotent high shelf only to the safe vocal track', () => {
  const { project, vocal, support } = projectWithVocal();
  const command = parseSectionVocalBrightnessCommand('dá mais brilho à minha voz só no refrão');
  const first = applySectionVocalBrightness(project, command, { now: 2000 });
  assert.equal(first.ok, true);
  assert.equal(first.event.kind, 'high_shelf');
  assert.equal(first.event.gainDb, 2.5);
  assert.equal(first.event.frequencyHz, 6500);
  assert.equal(first.event.startSeconds, 8);
  assert.equal(first.event.endSeconds, 16);
  assert.equal(first.event.source, PABLO_SECTION_VOCAL_BRIGHTNESS_SOURCE);
  assert.equal(first.project.tracks.find((track) => track.id === support.id).regionAutomation.length, 0);

  const second = applySectionVocalBrightness(first.project, command, { now: 3000 });
  const savedVocal = second.project.tracks.find((track) => track.id === vocal.id);
  assert.equal(savedVocal.regionAutomation.filter((event) => event.source === PABLO_SECTION_VOCAL_BRIGHTNESS_SOURCE).length, 1);
...
test('regional high shelf survives project migration with frequency metadata', () => {
  ...
  const applied = applySectionVocalBrightness(project, parseSectionVocalBrightnessCommand('dá mais brilho à minha voz no refrão'));
  ...
  assert.equal(event.kind, 'high_shelf');
  assert.equal(event.frequencyHz, 6500);
  assert.equal(event.gainDb, 2.5);
});
...
test('brightness uses confirmed-section and vocal ambiguity gates already established by section editing', () => {
  ...
  const command = parseSectionVocalBrightnessCommand('dá mais brilho à minha voz no refrão');
  const ambiguous = applySectionVocalBrightness(project, command);
  ...
  const resolved = applySectionVocalBrightness(project, command);
