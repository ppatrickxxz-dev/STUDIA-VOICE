...
test('cleanup adapter decodes the vocal once through canonical analyzeAudioTrack', async () => {
  ...
  assert.doesNotMatch(adapter, /detectBreathAndSibilance|detectPlosives|detectVocalPeaks|decodeAudioData/);
  assert.match(adapter, /A limpeza vocal não foi confirmada/);
});

test('cleanup core delegates sibilance plosive and dynamics to existing canonical planners', async () => {
...
test('vocal peak evidence is a canonical fourth voice event family', async () => {
