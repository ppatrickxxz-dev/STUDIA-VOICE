...
test('rewrite without current lyrics and without a pending draft is blocked before any network callback', async () => {
  let calls = 0;
  const result = await executePabloAudioMessage('Reescreve esse refrão sem perder meu jeito', {
    projectId: 'local-rewrite-empty',
    lyrics: '',
  }, {
    generateMusicDraft: async () => { calls += 1; return { text: 'não deveria rodar' }; },
  });
...
test('conversation UI uses stored draft acceptance once, then clears revision state only after drift-safe persistence', async () => {
  ...
  assert.match(source, /const saved = applyConfirmedPmiDraft\(project, \{ \.\.\.confirmation, draftVersion \}, snapshotProject\)/);
  assert.match(source, /await persistProject\(saved\);\s*clearPmiPendingDraft\(saved\.id\);\s*clearPmiDraftApplyState\(saved\.id\);\s*await clearPmiComposerState\(saved\.id\);/s);
