...
test('large candidate-model parts use signed TUS with exact 6 MiB chunks', () => {
  ...
  assert.match(worker, /\.storage\.supabase\.co\/storage\/v1\/upload\/resumable/)
...
test('existing four-part artifact contract stays unchanged and only transport changes', () => {
  ...
  assert.match(worker, /worker_version':'voice-train-v1-budget20-exact-checkpoint-recovery-applio-config-init-tus6m'/)
})
