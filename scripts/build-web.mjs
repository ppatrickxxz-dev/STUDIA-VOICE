...
// packages/app is copied into the Web root. Source imports that correctly reach
// sibling packages with ../ become one level too high after that flattening.
// Rewrite only root-level app modules in the built artifact; source modules and
// package-internal relative imports remain untouched.
...
async function assertBuiltRelativeImportsResolve(directory) {
  ...
  if (missing.length) {
    throw new Error(`WEB_BUILD_UNRESOLVED_IMPORTS\n${missing.join('\n')}`);
  }
}
