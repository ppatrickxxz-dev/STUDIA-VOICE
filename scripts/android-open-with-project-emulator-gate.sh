...
assert_project_imported() {
  local label="$1"
  forward_webview_devtools
  PV_ASSERT_LABEL="$label" python3 > "$evidence_dir/${label}-project-devtools.txt" <<'PY'
import base64
import json
import os
import socket
...
pages = json.loads(urllib.request.urlopen('http://127.0.0.1:9224/json', timeout=10).read().decode('utf-8'))
page = next((item for item in pages if item.get('type') == 'page'), pages[0])
ws_url = page['webSocketDebuggerUrl']
assert ws_url.startswith('ws://127.0.0.1:9224') or ws_url.startswith('ws://localhost:9224')
      ...
      try {
        ...
        if (match && asset && blobSize > 44 && Number(match.track.duration) > 0 && Number(match.track.sampleRate) > 0) {
          resolve({
            ...
            blobSize,
            visible: text.includes(match.track.name),
          });
          return;
        }
        if (Date.now() - started > 30000) {
          reject(new Error(`PROJECT_IMPORT_TIMEOUT projects=${projects.length} audio=${audio.length} hasMatch=${Boolean(match)} blobSize=${blobSize}`));
          return;
        }
        setTimeout(poll, 500);
      } catch (error) {
...
while True:
    ...
    if data.get('id') != 1:
        continue
    print(json.dumps({'label': label, 'data': data}, sort_keys=True))
