const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export const HARMONY_ROUTE_CONTRACT = Object.freeze({
  endpoint: 'progress-kaggle-harmony-v73',
  engine: 'Rubber Band + librosa',
  modes: Object.freeze(['texture', 'adaptive', 'adaptive_partial']),
  voices: Object.freeze(['high', 'low']),
  formantPreservationRequired: true,
});

export function buildHarmonyPairPlan({ mode = 'adaptive_partial' } = {}) {
  if (!HARMONY_ROUTE_CONTRACT.modes.includes(mode)) throw new Error(`Unsupported harmony mode: ${mode}`);
  return Object.freeze([
    Object.freeze({ voice: 'high', mode }),
    Object.freeze({ voice: 'low', mode }),
  ]);
}

export class KaggleHarmonyClient {
  constructor({ supabaseUrl, publishableKey, endpointSlug = HARMONY_ROUTE_CONTRACT.endpoint, fetchImpl = globalThis.fetch } = {}) {
    this.supabaseUrl = String(supabaseUrl || '').replace(/\/$/, '');
    this.publishableKey = String(publishableKey || '');
    this.endpointSlug = String(endpointSlug || HARMONY_ROUTE_CONTRACT.endpoint);
    this.fetch = fetchImpl;
  }

  async dispatch({ accessToken, projectId, voice, mode = 'adaptive_partial', semitones = null } = {}) {
    this.#validateRuntime(accessToken, projectId);
    if (!HARMONY_ROUTE_CONTRACT.voices.includes(voice)) throw new Error('Harmony voice must be high or low.');
    if (!HARMONY_ROUTE_CONTRACT.modes.includes(mode)) throw new Error('Unsupported harmony mode.');

    const body = { action: 'dispatch', project_id: projectId, voice, mode };
    if (mode === 'texture' && Number.isFinite(Number(semitones))) body.semitones = Number(semitones);
    const payload = await this.#request(accessToken, body);
    if (!payload?.ok || !UUID_RE.test(String(payload.job_id || ''))) throw new Error(payload?.error || 'Harmony dispatcher returned an invalid job.');
    if (payload.voice != null && payload.voice !== voice) throw new Error('Harmony dispatcher changed the requested voice.');
    if (payload.mode != null && payload.mode !== mode) throw new Error('Harmony dispatcher changed the requested mode.');
    return payload;
  }

  async status({ accessToken, projectId } = {}) {
    this.#validateRuntime(accessToken, projectId);
    return this.#request(accessToken, { action: 'status', project_id: projectId });
  }

  async sync({ accessToken, projectId } = {}) {
    this.#validateRuntime(accessToken, projectId);
    return this.#request(accessToken, { action: 'sync', project_id: projectId });
  }

  #validateRuntime(accessToken, projectId) {
    if (!this.supabaseUrl || !this.publishableKey) throw new Error('Supabase runtime configuration is required.');
    if (!accessToken) throw new Error('Authenticated access token is required.');
    if (!UUID_RE.test(String(projectId || ''))) throw new Error('Valid projectId is required.');
    if (typeof this.fetch !== 'function') throw new Error('Fetch transport is unavailable.');
  }

  async #request(accessToken, body) {
    const response = await this.fetch(`${this.supabaseUrl}/functions/v1/${this.endpointSlug}`, {
      method: 'POST',
      headers: {
        apikey: this.publishableKey,
        authorization: `Bearer ${accessToken}`,
        'content-type': 'application/json',
      },
      body: JSON.stringify(body),
    });
    const text = await response.text();
    let payload;
    try { payload = text ? JSON.parse(text) : null; }
    catch { payload = { error: text.slice(0, 500) }; }
    if (!response.ok) throw new Error(payload?.error || `Harmony request failed (${response.status}).`);
    return payload;
  }
}

export function classifyHarmonyPairReadiness({ highEvidence = null, lowEvidence = null, routeAvailable = true } = {}) {
  const routeImplemented = routeAvailable === true
    && HARMONY_ROUTE_CONTRACT.voices.includes('high')
    && HARMONY_ROUTE_CONTRACT.voices.includes('low')
    && HARMONY_ROUTE_CONTRACT.formantPreservationRequired === true;
  const highValidated = highEvidence?.state === 'validated' && highEvidence?.promotable === true;
  const lowValidated = lowEvidence?.state === 'validated' && lowEvidence?.promotable === true;
  const pairValidated = routeImplemented && highValidated && lowValidated;
  const blockers = [];
  if (!routeImplemented) blockers.push('harmony_route_unavailable');
  if (!highValidated) blockers.push(highEvidence ? 'high_acoustic_evidence_failed' : 'high_acoustic_evidence_missing');
  if (!lowValidated) blockers.push(lowEvidence ? 'low_acoustic_evidence_failed' : 'low_acoustic_evidence_missing');
  return Object.freeze({
    routeImplemented,
    highValidated,
    lowValidated,
    pairValidated,
    promotable: pairValidated,
    blockers,
    state: pairValidated ? 'pair_validated' : routeImplemented ? 'pair_evidence_pending' : 'incomplete',
  });
}
