...
export function critiqueLyrics(text = '', { concept = null, authorialMemory = null } = {}) {
  const lyrics = analyzeLyrics(text);
  const rhyme = analyzeRhymeArchitecture(text);
  const normalized = normalizePortuguese(text);
  const clicheHits = CLICHES.filter((item) => normalized.includes(normalizePortuguese(item)));
  const repeated = repetitionRatio(lyrics.lines || []);
  const authorial = evaluateAuthorialFit(text, authorialMemory || {});
  const issues = [];
  ...
  if (clicheHits.length) issues.push(issue('cliche','medium',`Há formulações previsíveis: ${clicheHits.join(', ')}.`));
  ...
  return Object.freeze({
    severity: issues.some((item) => item.severity === 'high') ? 'review' : 'info',
    findings: issues.map((item) => item.action),
    ...
    metrics: Object.freeze({
      meterConsistency: lyrics.meterConsistency,
      rhymeCoverage: lyrics.rhymeCoverage,
      singability: lyrics.singability,
    }),
    issues,
