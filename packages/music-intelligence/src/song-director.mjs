...
function score(candidate,plan){const c=candidate.energyCurve||[],chorus=avg(c.filter(x=>x.section==='chorus').map(x=>x.energy),.8),verse=avg(c.filter(x=>x.section==='verse').map(x=>x.energy),.45),bridges=c.filter(x=>x.section==='bridge').map(x=>x.energy),bridge=avg(bridges,.52),contrast=clamp((chorus-verse)/.5,0,1),bridgeContrast=bridges.length?clamp(Math.abs(chorus-bridge)/.45,0,1):.7,promptCoverage=Math.min(1,String(plan.artistBrief||plan.brief||'').trim().length/80+.35),evolution=candidate.providerDirection.includes('avoid loop-like repetition')?1:0,total=Number((contrast*32+bridgeContrast*18+promptCoverage*20+evolution*20+10).toFixed(2));return Object.freeze({total,sectionContrast:Number(contrast.toFixed(3)),bridgeContrast:Number(bridgeContrast.toFixed(3)),promptCoverage:Number(promptCoverage.toFixed(3)),evolution})}
...
function sectionId(value){const text=String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z]/g,'');if(text.includes('intro'))return'intro';if(text.includes('outro'))return'outro';if(text.includes('posrefr')||text.includes('postchorus')||text.includes('post'))return'post';if(text.includes('pre'))return'pre';if(text.includes('chorus')||text.includes('refrao')||text.includes('refr'))return'chorus';if(text.includes('verse')||text.includes('verso'))return'verse';if(text.includes('bridge')||text.includes('ponte')||text.includes('rap'))return'bridge';if(text.includes('break')||text.includes('cut')||text.includes('pickup'))return'breakdown';return text||'verse'}
