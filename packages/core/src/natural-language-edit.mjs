...
export function interpretNaturalLanguageEdit(command) {
  ...
  if (/\b(limpa|limpar|mais limpa|limpeza)\b/.test(text)) operations.push({ type: 'set_effect', key: 'clean', value: true, evidence: 'clean' });
  if (/\b(presente|presenca|mais presenca|mais presente)\b/.test(text)) operations.push({ type: 'set_effect', key: 'presence', value: true, evidence: 'presence' });
  if (/\b(quente|calor|mais quente)\b/.test(text)) operations.push({ type: 'set_effect', key: 'warm', value: true, evidence: 'warm' });
  if (/\b(sibilancia|sibilante|esses|de-?esser)\b/.test(text) && /\b(menos|reduz|tirar|tira|controla|suaviza)\b/.test(text)) operations.push({ type: 'set_effect', key: 'deEsser', value: true, evidence: 'deEsser' });
  if (/\b(centraliza|centralizada|centralizado|no centro|centro)\b/.test(text)) operations.push({ type: 'set_track', key: 'pan', value: 0, evidence: 'pan_center' });
  if (/\bfade\b/.test(text) && /\b(comeco|inicio|entrada)\b/.test(text) && /\b(curto|rapidinho|bem curto)\b/.test(text)) operations.push({ type: 'set_effect', key: 'fadeIn', value: SHORT_FADE_SECONDS, evidence: 'short_fade_in' });

  if (hasPreservationTarget(text, ['fim', 'final', 'saida'])) {
    preserved.add('trimEnd');
    preserved.add('fadeOut');
  }
  if (hasPreservationTarget(text, ['comeco', 'inicio', 'entrada'])) preserved.add('trimStart');

  if (!operations.length) return { supported: false, normalized_command: text, operations: [], preserved: [...preserved], reason: 'Nenhuma operação segura e determinística foi reconhecida.' };
  return { supported: true, normalized_command: text, operations, preserved: [...preserved], reason: null };
}
...
export function executeNaturalLanguageEdit(project, command, { trackId = null, now = Date.now() } = {}) {
  ...
  const before = { trimStart: track.trimStart, trimEnd: track.trimEnd, fadeOut: track.effects.fadeOut };
  ...
  track.updatedAt = now;
  next.updatedAt = now;
  return { project: next, track_id: track.id, intent, applied_operations: intent.operations, preserved: intent.preserved };
}
...
export function executeNaturalLanguageEditSequence(project, commands, { trackId = null, now = Date.now() } = {}) {
  ...
  commands.forEach((command, index) => {
    ...
    steps.push({ index: index + 1, command, applied_operations: result.applied_operations, preserved: result.preserved });
  });
...
export const NATURAL_LANGUAGE_EDIT_LIMITS = Object.freeze({
  ...
  supported_operations: Object.freeze([
    'clean_on', 'presence_on', 'warm_on', 'de_esser_on', 'pan_center', 'short_fade_in',
    'preserve_trim_start', 'preserve_trim_end', 'preserve_fade_out', 'sequential_continuity_guard',
  ]),
