...
export async function executeReviewedSectionRegeneration(review = {}, project = {}, {
  client = null,
  persist = persistSectionRegeneration,
  playhead = null,
  now = Date.now(),
} = {}) {
  ...
  let persisted;
  try {
    persisted = await persist(project, prepared.providerPlan, result);
  } catch (error) {
    return Object.freeze({
      ok: false,
      mutated: false,
...
export function humanizeReviewedGenerationError(reason = '') {
  const messages = {
    section_regeneration_plan_required: 'Esse pedido não terminou em uma regeneração de seção executável.',
    preserve_unselected_required: 'Não vou regenerar a faixa inteira quando o pedido é local. O restante precisa permanecer preservado.',
    inpainting_source_missing: 'Ainda não existe um take HQ com ID de continuidade para refazer só essa seção.',
    ...
    auth_required: 'Conecte sua sessão do PabloVoice para gerar a nova versão; o take atual foi preservado.',
    provider_unavailable: 'O motor Music v2 não está disponível agora; o take atual foi preservado.',
    provider_rate_limited: 'O motor Music v2 atingiu o limite temporário; o take atual foi preservado.',
    provider_auth_failed: 'A credencial do motor Music v2 precisa ser corrigida no backend; nada foi substituído.',
    provider_request_rejected: 'O motor Music v2 recusou esse plano; nada foi substituído.',
    invalid_inpainting_plan: 'O intervalo não pôde ser enviado com segurança para regeneração.',
    section_regeneration_persist_failed: 'A geração respondeu, mas o novo take não pôde ser salvo; o projeto anterior foi preservado.',
  ...
  return messages[String(reason || '')] || 'Não consegui concluir essa regeneração com segurança; o take atual foi preservado.';
}
