...
function scanReply(command, result) {
  ...
  if (result.clean) {
    return `Ouvi ${where} e não encontrei respiração forte, sibilância, estouro de P/B, estalos curtos, picos, ruído, hum ou reflexo de ambiente acima dos gates atuais. Foi só diagnóstico: não alterei nada.`;
  }
  ...
  const details = result.findings.slice(0, 8).map((finding) => {
    ...
    const level = Number.isFinite(Number(finding.rmsDb)) ? ` · nível ${Number(finding.rmsDb).toFixed(1)} dBFS` : '';
    const stationarity = Number.isFinite(Number(finding.stationarity)) ? ` · estável ${Math.round(Number(finding.stationarity) * 100)}%` : '';
    ...
    const classifiedNoise = finding.type === 'hum' || finding.type === 'broadband_noise';
    const action = classifiedNoise
      ? 'classificação diagnóstica; sem filtro dedicado automático'
      : finding.autoEdit ? 'tratável pelo cleanup atual' : 'revisar antes de editar';
    return `${finding.label} em ${at} · ${confidence}%${frequency}${noise}${level}${stationarity}${reflection}${timbre} · ${action}`;
  });
  const omitted = Math.max(0, result.findings.length - details.length);
  const observed = result.observed || {};
  const counts = `respirações ${observed.breaths || 0}, sibilâncias ${observed.sibilance || 0}, P/B ${observed.plosives || 0}, estalos ${observed.clicks || 0}, picos ${observed.peaks || 0}, ruído restaurável ${observed.noiseWindows || 0}, reflexo ${observed.reverbWindows || 0}, ruído classificado ${observed.classifiedNoise || 0}, hum ${observed.hum || 0}`;
  const hasClassifiedNoise = result.findings.some((finding) => finding.type === 'hum' || finding.type === 'broadband_noise');
  const classificationNote = hasClassifiedNoise
    ? ' A classificação hum/broadband é somente diagnóstica: o cleanup pode reduzir o piso geral apenas quando o restoration v9 aprova a proteção de timbre; não apliquei notch de 50/60 Hz automaticamente.'
    : '';
  return `Ouvi ${where} sem alterar nada. Encontrei ${result.findings.length} ponto(s) acima dos gates atuais (${result.actionableCount} acionável(is), ${result.reviewCount} para revisão). ${details.join('; ')}${omitted ? `; +${omitted} ponto(s) não listado(s)` : ''}. Evidência observada no trecho: ${counts}.${classificationNote}`;
}
