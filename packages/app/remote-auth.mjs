...
export class RemoteAuthAdapter {
 ...
 consumeBootstrapFragment(){const hash=String(this.location?.hash||'');if(!hash.includes('access_token=')&&!hash.includes('device_token='))return false;const params=new URLSearchParams(hash.replace(/^#/,''));const nextSession=sessionFromPayload(Object.fromEntries(params));const nextDeviceToken=String(params.get('device_token')||'');if(nextSession)this.setSession(nextSession);if(nextDeviceToken)this.setDeviceToken(nextDeviceToken);if(this.location?.history?.replaceState)this.location.history.replaceState(null,'',`${this.location.pathname||'/'}${this.location.search||''}`);else if(globalThis.history?.replaceState)globalThis.history.replaceState(null,'',`${this.location?.pathname||'/'}${this.location?.search||''}`);this.status=nextSession?'authenticated':this.status;return Boolean(nextSession||nextDeviceToken);}
 setSession(session){this.session=session;this.storage?.setItem?.(SESSION_KEY,JSON.stringify(session));}
 ...
 async loginWithEmail(email){const value=String(email||'').trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value))throw new Error('Digite um e-mail válido.');const redirectTo=`${String(this.location?.origin||CLOUDFLARE_RUNTIME_URL).replace(/\/$/,'')}/`;const response=await this.fetch(`${PROJECT_URL}/auth/v1/otp`,{method:'POST',headers:authHeaders(),body:JSON.stringify({email:value,create_user:true,options:{emailRedirectTo:redirectTo}})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(String(data?.error_description||data?.msg||data?.message||`email_login_${response.status}`));this.status='email-sent';return {ok:true,email:value};}
