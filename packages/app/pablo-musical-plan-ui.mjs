...
function capturePabloMessage(event) {
  ...
  pendingReview = { token, source, promise: prepareReview(source) };
}
...
async function decoratePlanMessage(message, review) {
  ...
  if (isSectionGenerationPlan(plan)) {
    ...
    const prepared = project
      ? prepareReviewedSectionRegeneration(review, project)
...
function renderGenerationActions(message, review, prepared) {
  ...
  target.textContent = `${prepared.targetSection.label} · ${formatSeconds(prepared.targetSection.startSeconds)} · ${prepared.targetSource === 'recent_studio_playhead' ? 'pelo ponto que você estava ouvindo' : 'seção confirmada no mapa'}`;
  message.appendChild(target);

  const safety = document.createElement('small');
  safety.dataset.musicalGenerationSafety = 'true';
  safety.textContent = '〰 Wave vai criar outro take só dessa seção. O restante e a versão anterior ficam preservados.';
  message.appendChild(safety);
  ...
  generate.addEventListener('click', async () => {
    ...
    generate.textContent = '〰 Criando seção…';
    try {
      const project = await currentProject();
      if (!project) throw new Error(humanizeMusicalReviewError('project_required'));
      ...
      completed = true;
      generate.textContent = 'Nova versão salva ✓';
      generate.disabled = true;
      appendGeneratedAudio(message, result);
      appendStatus(`${result.section?.label || 'Seção'} ganhou uma nova versão no Take ${result.takeNumber}. O take anterior e o restante da música continuam intactos.`);
      ...
      generate.textContent = '〰 Gerar nova versão';
      appendStatus(error?.message || 'Não consegui criar essa seção. O take anterior foi preservado.', true);
    }
  });
...
function executorLabel(executor) {
  return ({
    ...
    beat_lab: 'Beat Lab',
    music_generation: 'Wave · edição por seção',
