import {
  ...
  resolveSectionVocalClickTarget,
} from './core/src/section-vocal-click.mjs';
...
export function installPabloSectionVocalClickAdapter() {
  if (mounted) return;
  mounted = true;
...
async function onPabloSubmitCapture(event) {
  ...
  const command = parseSectionVocalClickCommand(original);
  if (!command) return;
  ...
  try {
    ...
    const clickEvents = analysis?.voice?.clickEvents;
    const analysisSource = analysis?.voice?.eventDetection?.source;
    if (!Array.isArray(clickEvents)) throw new Error('A análise de estalos não retornou evidência utilizável. Não alterei a voz.');
    const result = applySectionVocalClick(project, command, { clickEvents, analysisSource, now: Date.now() });
    if (!result.ok) {
      appendMessage(blockedPlanReply(command, result), 'assistant', { canApply: false });
      return;
    }
    const snapshotted = snapshotProject(result.project, `Estalos vocais no ${result.section.label}`);
    ...
    const savedEvents = (savedTrack?.regionAutomation || []).filter((item) =>
      item?.source === PABLO_SECTION_VOCAL_CLICK_SOURCE && String(item?.id || '').endsWith(`:${result.section.id}`));
    ...
    })) throw new Error('O tratamento de estalos não foi confirmado no projeto local. Não vou dizer que foi aplicado.');
    globalThis.dispatchEvent(new CustomEvent('pablovoice:project-persisted', {
      detail: { projectId: project.id, source: PABLO_SECTION_VOCAL_CLICK_SOURCE },
    }));
    appendMessage(
      `Encontrei ${result.detectedCount} estalo(s) curto(s) ${trackLabel(result.track)} no ${occurrenceLabel(command, result.section)} e atenuei só essas microjanelas. Plosivas e picos vocais maiores ficaram fora desse tratamento. Redução máxima: ${result.maxReductionDb.toFixed(1)} dB.`,
      'assistant',
      { canApply: true },
    );
  } catch (error) {
    appendMessage(error?.message || 'Não consegui tratar os estalos com segurança.', 'assistant', { error: true, canApply: false });
  } finally {
    setBusy(form, false);
  }
}
...
function blockedCommandReply(command) {
  if (command.reason === 'click_out_of_safe_range') return `Você pediu até ${command.requestedReductionDb} dB de redução. Neste gate eu só aplico automaticamente entre 0,5 e 7 dB em estalos muito curtos. Não alterei o projeto.`;
  return 'Não consegui transformar esse pedido em tratamento regional de estalos seguro. Não alterei o projeto.';
}
function blockedPlanReply(command, result) {
  ...
  if (result.reason === 'no_click_evidence') return `Não encontrei estalos curtos de boca/microfone com evidência suficiente nesse ${command.label.toLowerCase()}. Não atenuei a voz por aproximação.`;
  if (result.reason === 'click_analysis_required') return 'Preciso analisar a própria faixa vocal antes de tratar estalos. Nenhuma microatenuação foi criada sem essa evidência.';
...
function appendMessage(text, role, result = null) {
  ...
  if (role === 'assistant' && result && !result.error) {
    ...
    meta.textContent = result.canApply ? 'Studio · estalos tratados' : 'Studio · edição não aplicada';
