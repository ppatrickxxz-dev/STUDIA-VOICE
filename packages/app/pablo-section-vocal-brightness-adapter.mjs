import {
  applySectionVocalBrightness,
  parseSectionVocalBrightnessCommand,
  PABLO_SECTION_VOCAL_BRIGHTNESS_SOURCE,
} from './core/src/section-vocal-brightness.mjs';
import { snapshotProject } from './core/src/project.mjs';
import { activeProjectSessionId, getProject, saveProject } from './storage.mjs';

let mounted = false;

export function installPabloSectionVocalBrightnessAdapter() {
  if (mounted) return;
  mounted = true;
...
async function onPabloSubmitCapture(event) {
  ...
  const command = parseSectionVocalBrightnessCommand(original);
  if (!command) return;
  ...
  try {
    ...
    const result = applySectionVocalBrightness(project, command);
    if (!result.ok) {
      appendMessage(blockedPlanReply(command, result), 'assistant', { canApply: false });
      return;
    }

    const label = `Brilho vocal ${signedDb(result.gainDb)} no ${result.section.label}`;
    ...
    const savedEvent = savedTrack?.regionAutomation?.find((item) => item.id === result.event.id && item.source === PABLO_SECTION_VOCAL_BRIGHTNESS_SOURCE);
    if (!savedEvent
      || savedEvent.kind !== 'high_shelf'
      || Math.abs(Number(savedEvent.gainDb) - result.gainDb) > 0.001
      || Math.abs(Number(savedEvent.frequencyHz) - result.frequencyHz) > 0.5) {
      throw new Error('O brilho regional não foi confirmado no projeto local. Não vou dizer que foi aplicado.');
    }

    globalThis.dispatchEvent(new CustomEvent('pablovoice:project-persisted', { detail: { projectId: project.id, source: PABLO_SECTION_VOCAL_BRIGHTNESS_SOURCE } }));
    appendMessage(
      `Dei mais brilho ${trackLabel(result.track)} somente no ${occurrenceLabel(command, result.section)}: high-shelf de ${signedDb(result.gainDb)} a partir de ${formatKhz(result.frequencyHz)} (${formatClock(result.range.timelineStartSeconds)} → ${formatClock(result.range.timelineEndSeconds)}). Volume, outras seções e outras faixas foram preservados.${result.range.clippedToTrack ? ' O efeito ficou limitado ao trecho em que essa voz realmente existe.' : ''}`,
      'assistant',
      { canApply: true },
    );
  } catch (error) {
    appendMessage(error?.message || 'Não consegui aplicar esse brilho com segurança.', 'assistant', { error: true, canApply: false });
  } finally {
    setBusy(form, false);
  }
}
...
function blockedCommandReply(command) {
  if (command.reason === 'brightness_out_of_safe_range') {
    return `Você pediu ${command.requestedGainDb} dB de brilho. Neste primeiro gate eu só aplico até +4 dB de high-shelf regional automaticamente. Não alterei o projeto.`;
  }
  return 'Não consegui transformar esse pedido em brilho regional seguro. Não alterei o projeto.';
}
...
function blockedPlanReply(command, result) {
  ...
  if (result.reason === 'missing_confirmed_end') return `${command.label} ainda não tem fim confirmado. Marque onde termina antes de mudar o brilho só dessa seção.`;
  if (result.reason === 'ambiguous_occurrence') return `Há ${result.sectionResult?.count || 'várias'} ocorrências de ${command.label.toLowerCase()}. Diga qual, por exemplo “dá mais brilho à minha voz só no primeiro ${command.label.toLowerCase()}”.`;
...
function appendMessage(text, role, result = null) {
  ...
  if (role === 'assistant' && result && !result.error) {
    ...
    meta.textContent = result.canApply ? 'Studio · brilho regional salvo' : 'Studio · edição não aplicada';
