  ...
  for (const remoteAsset of assets) {
    ...
    const track = {
      ...createTrack({
        ...
        type: remoteAsset.mime_type || 'audio/wav',
        duration: measured.duration,
        sampleRate: measured.sampleRate,
        channels: measured.channels,
      }),
      stemType: remoteAsset.kind === 'guide_vocal' ? 'vocal' : 'instrumental',
      renderJobId: job.id,
      remoteAssetId: remoteAsset.id,
      remoteProjectId: job.project_id,
      remoteSha256: downloaded.sha256,
      remoteCreatedAt: remoteAsset.created_at,
      provider: job.provider || 'kaggle',
      engine: job.proof?.engine || job.engine || 'Demucs',
      model: job.proof?.model || remoteAsset.metadata?.model || 'htdemucs',
    };
    ...
    imported.push({
      ...
      name: remoteAsset.original_name,
      format: remoteAsset.mime_type || 'audio/wav',
      duration: measured.duration,
      sampleRate: measured.sampleRate,
      channels: measured.channels,
