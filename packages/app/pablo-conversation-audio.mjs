export const CONVERSATIONAL_AUDIO_INTENTS = Object.freeze({ voice_forward: 'bring_voice_forward', make_vocal_space: 'make_vocal_space', soften_breaths: 'soften_breaths', remove_breaths: 'soften_breaths', align_vocals: 'align_vocals', audio_to_instrument: 'audio_to_instrument', inspect_audio: 'inspect_audio', inspect_mix: 'inspect_mix' });
...
export async function executePabloAudioMessage(message, context, { audioToolRuntime, executeDeterministicEdit } = {}) {
  const music = await tryMusicIntelligence(message, context); if (music?.supported) return { ...music, execution:'read_only', canApply:false };
  const parsed = interpretPabloAudioMessage(message, context); if (!parsed.supported) return parsed;
  if (parsed.kind === 'tool_call') { if (typeof audioToolRuntime !== 'function') throw new TypeError('audioToolRuntime is required for tool calls'); const result = await audioToolRuntime(parsed.tool, parsed.args); return { ...parsed, result, execution: result?.data?.execution || parsed.previewPolicy, canApply: result?.ok === true && result?.data?.execution === 'allowed' }; }
...
async function tryMusicIntelligence(message, context) {
  try { const { respondToAuthorialFeedback, respondToMusicCreation } = await import('./music-intelligence/src/index.mjs'); const feedback = respondToAuthorialFeedback(message, context); if (feedback?.supported) return feedback; return respondToMusicCreation(message, context); } catch { return null; }
}
