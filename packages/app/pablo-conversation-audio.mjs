  ...
  if (music?.supported) {
    ...
    if (music.kind === 'pmi_generation_request' || music.kind === 'pmi_draft_revision_request') {
      ...
      return {
        ...
        revisedPendingDraft: music.kind === 'pmi_draft_revision_request',
...
async function tryMusicIntelligence(message, context = {}) {
  ...
  const enrichedContext = projectId && pmiSessionCache.has(projectId) ? { ...context, pmiSession: pmiSessionCache.get(projectId) } : context;
  const feedback = intelligence.respondToAuthorialFeedback(message, enrichedContext);
  if (feedback?.supported) return feedback;
  const draftRevision = intelligence.planPendingDraftRevision?.(message, enrichedContext);
  if (draftRevision?.supported) return draftRevision;
...
async function loadMusicIntelligence() {
  for (const specifier of ['./music-intelligence/src/index.mjs', '../music-intelligence/src/index.mjs']) {
    try { return await import(specifier); } catch { /* Source tests and packaged runtime resolve the canonical package from different roots. */ }
  }
  return null;
}
