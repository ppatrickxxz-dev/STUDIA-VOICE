...
async function analyzeTrack(track) {
  ...
  try {
    ...
    const analysis = { schemaVersion: 2, assetId: track.assetId, source: { sampleRate: buffer.sampleRate, channels: buffer.numberOfChannels, durationSeconds: buffer.duration }, music: musical.music, signal: { ...waveform.signal, onsets, transients: onsets, spectralEnvelope }, voice: musical.voice, confidence: musical.confidence, validity: { complete: true, invalidatedRanges: [] } };
...
async function getMixState(projectId) {
  ...
  for (const track of project.tracks || []) {
    ...
    tracks.push({ trackId: track.id, role: track.kind === 'recording' ? 'lead-vocal' : 'instrumental', analysis, confidence: analysis.confidence?.voice ?? analysis.confidence?.pitch ?? 0 });
  }
...
async function applyBreathAutomation(result, trackId) {
  ...
  await persistProject(snapshotProject(project, 'Respirações ajustadas pelo Pablo'));
  return automatic;
}
...
async function contextForMessage() {
  ...
  return {
    projectId: project?.id || null, trackId: active?.id || null, assetId: active?.assetId || null,
    referenceAssetId: active?.assetId || null, targetAssetId: other?.assetId || null,
    lyrics: String(project?.lyrics || ''), notes: String(project?.notes || ''), preset: project?.preset || 'demo',
    genre: project?.genre || '', mood: project?.mood || '', authorialMemory: project?.authorialMemory || {},
  };
}

async function persistAuthorialMemory(result) {
  if (result?.kind !== 'pmi_authorial_feedback' || !result.authorialMemory) return false;
  const project = await activeProject();
  if (!project) return false;
  project.authorialMemory = result.authorialMemory;
  await persistProject(snapshotProject(project, 'Preferência autoral aprendida pelo Pablo'));
  return true;
}

function remoteContextPack(project) {
  const tracks = Array.isArray(project?.tracks) ? project.tracks : [];
  return { source: 'pablovoice-unified-local-first', project: { local_id: project?.id || null, title: project?.name || 'Projeto PabloVoice', preset: project?.preset || null, track_count: tracks.length, active_track_id: project?.activeTrackId || null, lyrics: String(project?.lyrics || '').slice(0, 12000), notes: String(project?.notes || '').slice(0, 4000), authorial_memory: project?.authorialMemory || {} }, tracks: tracks.slice(0, 16).map((track) => ({ id: track.id, name: track.name, kind: track.kind, duration: Number(track.duration || 0), gain: Number(track.gain ?? 1), pan: Number(track.pan || 0), muted: Boolean(track.muted), solo: Boolean(track.solo), effects: track.effects || {} })) };
}
...
async function tryRemoteReasoning(message) {
  ...
  const result = await remoteAuth.agentTurn({ project_id: linked.project.id, message, intent: { mode: 'advice_only', destructive_actions: false, source: 'unified_pablo_chat' }, context_pack: remoteContextPack(project), tools: [] });
  if (!result?.ok || !String(result.reply || '').trim()) return null;
  return { supported: true, kind: 'remote_reasoning', reply: String(result.reply).trim(), provider: result.provider || health.provider || 'remote', model: result.model || health.model || null };
}
...
async function submitMessage(form) {
  ...
  if (!message) return;
  input.value = ''; appendMessage(message, 'user'); setBusy(form, true);
  try {
    ...
    if (result?.supported) {
      ...
      if (result.kind === 'pmi_authorial_feedback') await persistAuthorialMemory(result);
      if (result.kind === 'deterministic_edit' && result.canApply) appendMessage('A edição determinística foi aplicada e salva. Reabra o Studio para ouvir a prévia atualizada.', 'assistant');
  ...
  } catch (error) { appendMessage(error?.message || 'Não consegui executar esse pedido com segurança.', 'assistant', { error: true }); }
...
function injectConversationBox() {
  ...
  injecting = true;
  const box = document.createElement('div'); box.dataset.pabloConversation = 'true'; box.className = 'pv-conversation';
  box.innerHTML = `<div class="pv-conversation-log" data-pablo-log><div class="pv-msg assistant">Pode falar do seu jeito: “deixa minha voz mais na frente”, “não use a palavra promessa”, “prefiro refrões mais curtos” ou “quero criar uma música sobre…”.<small>ações locais primeiro · IA remota opcional</small></div></div><form class="pv-compose-row" data-pablo-form><input class="pv-field" name="message" autocomplete="off" placeholder="O que você quer fazer com o som?" aria-label="Falar com Pablo sobre o áudio"><button class="pv-btn primary" type="submit">Enviar</button></form>`;
  shell.appendChild(box); box.querySelector('form').addEventListener('submit', (event) => { event.preventDefault(); submitMessage(event.currentTarget); }); injecting = false;
}

function appendMessage(text, role, result = null) {
  const log = document.querySelector('[data-pablo-log]'); if (!log) return;
  const message = document.createElement('div'); message.className = `pv-msg ${role}`; message.textContent = text;
  let metaText = '';
  ...
  else if (result?.kind === 'pmi_music_session') metaText = 'PMI Music Intelligence · contexto do projeto';
  else if (result?.kind === 'pmi_authorial_feedback') metaText = 'memória autoral · salva neste projeto';
  if (metaText) { const meta = document.createElement('small'); meta.textContent = metaText; message.appendChild(meta); }
  log.appendChild(message); message.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
}

function formatResult(result) {
  if (!result?.supported) return 'Ainda não tenho uma ação segura para esse pedido. Não alterei o projeto.';
  if (result.kind === 'pmi_music_session' || result.kind === 'pmi_authorial_feedback') return result.reply || 'PMI processou o pedido.';
  ...
  if (result.tool === 'inspect_audio') { const bpm = featureValue(data.music?.bpm ?? data.music?.bpm?.value); const pitch = featureValue(data.voice?.pitchHz ?? data.voice?.pitchHz?.value); return `Analisei o áudio${bpm ? `: cerca de ${Math.round(bpm)} BPM` : ''}${pitch ? `, pitch central ~${Math.round(pitch)} Hz` : ''}.`; }
...
function coarseSpectrum(samples, sampleRate, bands = 8) {
  const size = Math.min(2048, samples.length); if (size < 64) return [];
  const start = Math.max(0, Math.floor((samples.length - size) / 2)); const frame = samples.subarray(start, start + size); const nyquist = sampleRate / 2; const values = [];
  for (let band = 0; band < bands; band += 1) { const low = 40 * Math.pow(nyquist / 40, band / bands); const high = 40 * Math.pow(nyquist / 40, (band + 1) / bands); const center = Math.sqrt(low * high); const k = Math.max(1, Math.min(Math.floor(size / 2) - 1, Math.round(center * size / sampleRate))); let re = 0, im = 0; for (let n = 0; n < size; n += 1) { const angle = 2 * Math.PI * k * n / size; const window = 0.5 - 0.5 * Math.cos(2 * Math.PI * n / (size - 1)); re += frame[n] * window * Math.cos(angle); im -= frame[n] * window * Math.sin(angle); } values.push(Math.sqrt(re * re + im * im)); }
  const max = Math.max(...values, 1e-9); return values.map((value, index) => ({ band: index, value: value / max }));
}

function featureValue(value) { const n = Number(value?.value ?? value); return Number.isFinite(n) ? n : null; }
function setBusy(form, busy) { const button = form.querySelector('button'); const input = form.querySelector('input'); if (button) { button.disabled = busy; button.textContent = busy ? 'Analisando…' : 'Enviar'; } if (input) input.disabled = busy; }
