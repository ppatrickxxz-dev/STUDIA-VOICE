import { migrateProject, snapshotProject } from '../core/src/project.mjs';
...
export function applyPabloInstrumentOperation(project, plan = {}) {
  ...
  // Preserve a real pre-edit state even when the user previously saved the Instrument Lab
  // without creating a project revision.
  const baseline = snapshotProject(migrateProject(project), 'Antes do ajuste inteligente de instrumento');
  const next = migrateProject(baseline);
  next.instrumentLab = normalizeInstrumentState({
    ...current,
    notes: transformedNotes,
  });
  const saved = snapshotProject(next, 'Instrumento ajustado pelo Pablo');
